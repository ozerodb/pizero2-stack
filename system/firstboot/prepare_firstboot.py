...
def _parse_kv(path: Path) -> dict[str, str]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        raw = raw.strip()
        if raw and raw[0] in ('"', "'"):
            # Quoted value — preserve everything between the matching quotes verbatim.
            # Allows passwords with #, spaces, and other special characters.
            q = raw[0]
            end = raw.find(q, 1)
            val = raw[1:end] if end != -1 else raw[1:]
        else:
            # Unquoted — strip trailing inline comment (# and beyond).
...
def build(here: Path, repo: Path, kv: dict[str, str]) -> None:
    ...
    #    Bcrypt hashes for ntfy users (ntfyadmin + ntfypub) are computed here.
    ...
    for key in DOCKER_ENV_KEYS:
        val = kv.get(key, ENV_DEFAULTS.get(key, ""))
        # Bcrypt hashes contain $ — escape as $$ so Docker Compose doesn't
        # double-interpolate after substituting the value into docker-compose.yml.
        if key in ("NTFY_ADMIN_HASH", "NTFY_PUBLISH_HASH") and val:
            val = val.replace("$", "$$")
